...
int main(int argc, char **argv)
{
	...
	IloNumVar theta_a(env),theta_b(env);
	IloNum thetasol_a =0 ,thetasol_b = 0;
	IloNumVar u(env);IloNum usol = 0.5;
	model_master.add(IloMinimize(env,theta_a + theta_b + 10*u));
	...
	while ((UB-LB) > 0.0001)
	{
		...
		UB = IloMin (UB,ubound);
		model_master.add(theta_a >= (16-8*u)*xsol1+(10-2*u)*xsol2);
		model_master.add(theta_b >= (4-4*u)*xsol4-u*xsol3);
		...
		cout << "Cuts          = "<<"theta_a >= "<<xsol1<<"(16-8u)+"<<xsol2<<"(10-2u)\n";
		cout << "                "<<"theta_b >= -"<<xsol3<<"u +"<<xsol4<<"(4-4u)\n";
		cout << "*****************************************************************************";
	}
}